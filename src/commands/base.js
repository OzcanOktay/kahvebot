...
class Command {
	...
	onError(err, message, args, fromPattern) { // eslint-disable-line no-unused-vars
		...
		const invite = "https://discord.gg/6Q8Geju";
		return message.reply(stripIndents`
			Bu komutu kullanırken bir hata oluştu: \`${err.name}: ${err.message}\`
			Eğer bu hatayı bilmiyorsan.
			Lütfen bu kişilerle iletişime geç ${ownerList || 'Bot Sahipleri'}${invite ? ` bu sunucuda: ${invite}` : '.'}
		`);
	}
